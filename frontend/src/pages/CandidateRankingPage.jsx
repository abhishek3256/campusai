            ...
            <div className="max-w-7xl mx-auto">
                ...
                <div className="grid grid-cols-1 lg:grid-cols-4 gap-6">
                    ...
                    <div>
                        ...
                        {loadingJobs ? <div className="flex justify-center py-4"><div className="w-6 h-6 border-2 border-amber-500 border-t-transparent rounded-full animate-spin" /></div>
                        : jobs.length === 0 ? <p className="text-sm text-gray-400 text-center py-4">No jobs posted yet</p>
                        : jobs.map(j => (
                            <button key={j._id} onClick={() => handleJobSelect(j)} className={`w-full text-left p-3 rounded-xl border mb-2 transition-all ${selectedJob?._id === j._id ? 'border-amber-500 bg-amber-50 dark:bg-amber-900/20' : 'border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 hover:border-amber-300'}`}>
                            ...
                            <div>
                                <div className="flex items-center justify-between mb-4">
                                    <div className="flex items-center gap-3">
                                        ...
                                        {shortlisted.size > 0 && <span className="px-2 py-1 bg-green-100 dark:bg-green-900/30 text-green-700 dark:text-green-400 rounded-full text-xs">{shortlisted.size} shortlisted</span>}
                                    </div>
                                    <div className="flex items-center gap-2">
                                        <div className="flex items-center gap-2 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-xl px-3 py-2">
                                            ...
                                            <input type="number" min={0} max={100} value={filter} onChange={e => setFilter(parseInt(e.target.value) || 0)} className="w-12 text-sm bg-transparent outline-none text-gray-900 dark:text-white" />
                                        </div>
                                        <button onClick={handleExportCSV} disabled={filtered.length === 0} className="flex items-center gap-1 px-3 py-2 bg-green-600 hover:bg-green-700 disabled:opacity-40 text-white rounded-xl text-sm">
                                ...
                                : filtered.length === 0 ? <div className="text-center py-16 text-gray-400"><Users className="w-12 h-12 mx-auto mb-3 opacity-30" /><p>No applicants match the filter</p></div>
                                : (
                                    <div className="space-y-3">
                                        {filtered.map((a, i) => {
                                            const s = a.studentId;
                                            const score = a.aiMatchScore || 0;
                                            const isShortlisted = shortlisted.has(a._id);
                                            return (
                                                <motion.div key={a._id} initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.03 }}
                                                    className={`bg-white dark:bg-gray-800 rounded-xl border-2 p-4 transition-all ${isShortlisted ? 'border-green-400 dark:border-green-600' : 'border-gray-200 dark:border-gray-700'}`}>
                                                    <div className="flex items-center gap-4">
                                                        {/* Rank */}
                                                        <div className={`w-8 h-8 rounded-xl flex items-center justify-center font-bold text-sm flex-shrink-0 ${i === 0 ? 'bg-yellow-100 text-yellow-700' : i === 1 ? 'bg-gray-100 text-gray-600' : i === 2 ? 'bg-orange-100 text-orange-600' : 'bg-gray-50 dark:bg-gray-700 text-gray-500'}`}>
                                                            {i === 0 ? '🥇' : i === 1 ? '🥈' : i === 2 ? '🥉' : i + 1}
                                                        </div>
                                                        ...
                                                        <div className="flex-1 min-w-0">
                                                            ...
                                                            {s?.aiSkills?.length > 0 && (
                                                                <div className="flex flex-wrap gap-1 mt-1">
                                                                    {s.aiSkills.slice(0, 5).map((sk, si) => <span key={si} className="px-2 py-0.5 bg-gray-100 dark:bg-gray-700 text-gray-600 dark:text-gray-400 rounded-full text-xs">{sk}</span>)}
                                                                    {s.aiSkills.length > 5 && <span className="text-xs text-gray-400">+{s.aiSkills.length - 5}</span>}
                                                                </div>
                                                            )}
                                                        </div>
                                                        ...
                                                        <div className="flex flex-col items-end gap-2">
                                                            ...
                                                            <button onClick={() => toggleShortlist(a._id)} className={`px-3 py-1 rounded-xl text-xs font-medium border transition-all ${isShortlisted ? 'bg-green-600 border-green-600 text-white' : 'border-gray-200 dark:border-gray-600 text-gray-600 dark:text-gray-400 hover:border-green-400'}`}>
                                                    ...
                                                    {a.missingSkills?.length > 0 && (
                                                        <div className="mt-2 flex flex-wrap gap-1">
                                                            ...
                                                            {a.missingSkills.map((ms, j) => <span key={j} className="px-2 py-0.5 bg-red-100 dark:bg-red-900/20 text-red-600 dark:text-red-400 rounded-full text-xs">{ms}</span>)}
                                                        </div>
                                                    )}
                                                </motion.div>
                                            );
